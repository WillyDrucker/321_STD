...
**Purpose:** Get everything committed and pushed to the project's anchored remote so the project stays current with its repo. The skill is deliberately thin - it loads the auto-push file (`autopush.config` in `_index.json`) and runs it. The substance lives in that file: the generic capture / commit / push baseline and the project's `## Project release steps`, which setup and reconcile enrich as the release cycle settles.
