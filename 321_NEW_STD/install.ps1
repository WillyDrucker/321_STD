...
if (-not ($Name -match '^[A-Za-z][A-Za-z0-9_-]*$')) {
  Write-Host "Invalid project name: '$Name'. Start with a letter; letters / digits / _ / - only." -ForegroundColor Red
...
Write-Host ""
Write-Host "321 installed at $Target" -ForegroundColor Green
Write-Host ""
Write-Host "Next steps:"
Write-Host "  cd `"$Target`""
Write-Host "  Open in your editor, then run /321 -Setup (optional first-run wizard or migration. The project is usable as-is)."
