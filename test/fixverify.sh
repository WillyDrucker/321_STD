...
# the shared runner (globals + pass/fail), builds the OLD321 legacy fixture once, then
# runs each domain-focused case file in order so the migration arc in 01 sets up the
# $PROJ state the doctor / state checks in 02 reuse. Per-test isolated projects live
# under $BASE in each case file. Run from the repo root: bash test/fixverify.sh
...
#   04-upgrade-ops.sh  - upgrade manifest operations (one isolated project per op kind)
#   05-upgrade-flow.sh - upgrade lifecycle (cleanup, fetch defaults, router, snapshot, hardening)
