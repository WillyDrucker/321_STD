...
| `import-skills.mjs` | Copies a project's own skill bodies into `AIDOCS/SKILL/` under the canonical `SKILL_<FUNC>.md` name so sync can register them with the router (which dispatches from `_index.json`, so no router edit is needed), and records each net-new body in `customizations[]` (provenance) so the no-conflict path needs no AI edit. Never overwrites a canonical body - a name collision is reported with the canonical hash for `/321 -Update` to merge as canonical base + delta. Non-destructive. `--from <dir>` overrides the default `AIDOCS/SKILLS/` scan: the migration points it at the setup archive, the reconcile late-scan at foreign skill dirs. |
