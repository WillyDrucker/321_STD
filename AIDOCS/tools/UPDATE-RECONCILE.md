...
- **Keep the user profile, replace the seed placeholder.** A filled `user_<name>.md` in the runtime is project data, not a canonical rule. The decision rule: if the seed still carries `user_name.md` (the placeholder) and the runtime carries a filled `user_<name>.md`, replace the seed file with the filled body and rename it to match, so a fresh-machine install restores the filled profile. Update the AGENTS Hard-rules pointer to the filled filename in the same edit. Doctor's auto-memory pointer check confirms the pointer resolves both ways.
