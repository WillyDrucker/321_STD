...
- **The file is the source of truth.** The skill stays thin. The release cycle lives in `AUTO-PUSH.md`, enriched at `-Setup` / reconcile, so an engine update never reverts a project's pipeline. **Apply what the file says, not what you remember it saying** - a wrapper that restates its file drifts from it.
