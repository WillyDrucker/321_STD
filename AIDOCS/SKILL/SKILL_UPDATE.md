...
**Skills lane (project `/321` skills).** Setup's migration already did the mechanical half: `import-skills` copied the project's net-new skill bodies into `AIDOCS/SKILL/` under the canonical `SKILL_<FUNC>.md` name (registered by the Step 3 sync), and reported any that collide with a canonical skill rather than overwriting it. This lane finalizes them. The source of truth for the project-specific deltas is the archived legacy tree (`AIDOCS/<X>_SETUP_ARCHIVE/AIDOCS/SKILLS/`, plus any project-customized `SKILL_*.md`). Run `node AIDOCS/tools/memory.mjs import-skills --from AIDOCS/<X>_SETUP_ARCHIVE/AIDOCS/SKILLS --dry-run` once to get the current classification and, for each collision, the canonical base hash. Then classify each project skill:

- **Net-new (already imported)** - a project skill with no canonical equivalent (Setup copied it to `AIDOCS/SKILL/SKILL_<FUNC>.md`). Finalize it: normalize the frontmatter `name` to a bare dispatch key by dropping the project prefix (`glp321-scrape-provider` becomes `scrape-provider`), voice-scrub the body, and record a `customizations[]` entry (`id`, `description`, `rule` naming it a project-authored skill with no canonical equivalent, `applies_to: ["AIDOCS/SKILL/SKILL_<FUNC>.md"]`). No `base` block - a body absent from the engine source is never overwritten by `init`, so there is nothing to track. The entry is provenance, the one list of every skill the project added.
- **Overlap (canonical base + delta)** - the project skill shares a function with a canonical one (auto-push and the like). `import-skills` kept the canonical body and reported the collision. Author the merge: keep the canonical body's universal spine and fold in only the project-specific steps and gates from the archived legacy body. Do NOT port the old body verbatim, it can be stale against the current spine. Apply the project rename and voice-scrub. Record a `customizations[]` entry that adds a `base` block naming the canonical skill and the hash from the report (`base: { "skill": "<dispatch-key>", "hash": "<canonical hash>" }`). That hash is what lets `init` nudge a re-merge once the canonical spine advances. `applies_to` names the body as for net-new.
...
Post-migration reconciliation complete. Gate cleared (reconcile_pending: false).
  ...
  Skills:    <T> project skill(s) - <I> net-new imported + registered, <A> overlap(s) merged
...
- **Reconciliation re-homes everything the migration captured.** When the gate is set, beyond SESSION / MEMORY / BACKLOG it also runs the skills lane (Setup imported the net-new project skills, this lane normalizes them, records `customizations[]`, merges overlaps as canonical base + delta with a recorded base hash, folds engine-superseded ones, and runs the late-scan finalize nudge), the AGENTS / CLAUDE classification lane (archived orchestrator content -> lean AGENTS / MEMORY / DEV-AUDIT), and the DEV-AUDIT Project-specifics dedup lane. All are direct edits gated by doctor. Routine (non-gated) updates touch none of these. The only manual step left is deleting the setup archive when satisfied.
