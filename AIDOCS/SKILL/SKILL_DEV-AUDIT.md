...
**Purpose:** Audit the source against `<PROJECT>_DEV-AUDIT.md` (registry key `devaudit.audit`). The skill is thin - it loads the audit file and applies it. The substance lives there: anchor principles, the hard-rules inventory mirrored from auto-memory, the audit dimensions, and the project's own `## Project specifics`, which `-Setup` and reconcile enrich as conventions settle.
