...
Audit-facing copy of the auto-memory inventory at `<auto_memory.path>`. The audit applies these whether or not auto-memory loaded. Same inventory, audit-facing wording.
...
Stack-specific conventions, framework patterns, deployment idioms, language-specific style rules below this line. The baseline above applies to every project using this template. Architecture / decisions / rules tied to the codebase identity live in `<PROJECT>_MEMORY.md`, not here.
