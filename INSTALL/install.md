...
4. **Scaffold.** `init <target> --name <NAME> [--privacy <mode>] [--upstream <url>]` lays the skeleton - engine, skills, auto-memory, data files with the name substituted, and `INSTALL/` with these runbooks. The `.gitignore` is generated for the privacy mode (not copied), so a public install gates the project's own knowledge from the start. A reinstall (the migration relay) recalls the mode from the registry, then the archive, so it is not lost. `--upstream` records the install source into `engine.upstream` write-if-empty, so a later `/321 -UpdateSync` knows where to pull from. The bootstrap scripts always pass `--upstream`. An AI-driven install (the `321done.ai/std` path) also passes it, with the source repo URL the install fetched from.
