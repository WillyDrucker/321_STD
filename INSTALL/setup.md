...
4. **Reinstall the canonical structure.** Lay fresh scaffolds over the now-empty shape, keeping the engine in place and re-laying the canonical auto-memory. `init` refuses to scaffold over its own source, so the reinstall runs from a fetched engine, not the project's own copy:
   ```bash
...
6. **Set the reconcile gate (capture stays additive).**
   ```bash
...
9. **MEMORY capture (script 1:1, then the initial project check).** Same shape.
...
10. **Scrub and verify the capture.**
    ```bash
